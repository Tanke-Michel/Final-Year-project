...
def pdf_has_mock(path: Path) -> bool:
    raw = path.read_bytes()
    # 1. Metadata stamp written by src/figures.py --mock. Stored uncompressed,
    #    independent of how the figure's text is drawn. This is the reliable test.
    if MARKER.encode() in raw:
        return True
    # 2. Visible watermark text. Works only when matplotlib writes text as
    #    literal characters: 3.10 does, 3.11 writes glyph codes and this finds
    #    nothing. Kept as a fallback for figures produced before the stamp.
    if b"MOCK" in raw:
        return True
...
def main() -> int:
    ...
    for f in sorted(list(d.glob("*.tex")) + list(d.glob("*.md"))):
        if MARKER in f.read_text(encoding="utf-8", errors="replace"):
            problems.append(f"mock table: {f.name}")
    ...
    for f in sorted(list(d.glob("*.pdf")) + list(d.glob("*.png"))):
        try:
            ...
            elif f.suffix == ".png" and (MARKER.encode() in f.read_bytes()
                                         or b"MOCK" in f.read_bytes()):
                problems.append(f"mock watermark in {f.name}")
