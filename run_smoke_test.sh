...
fig, ax = plt.subplots(); fig.text(.5,.5,"MOCK DATA",fontsize=40,alpha=.2)
fig.savefig("/tmp/st_mock/fake.pdf")
PYEOF
python3 scripts/check_mock.py /tmp/st_mock >/dev/null 2>&1 \
  && bad "mock detector missed a watermarked PDF" \
  || ok "mock detector catches compressed watermarks"
rm -rf /tmp/st_clean && mkdir -p /tmp/st_clean
python3 scripts/check_mock.py /tmp/st_clean >/dev/null 2>&1 \
  && ok "mock detector passes a clean directory" \
  || bad "mock detector false-positives on a clean directory"
